-- ─────────────────────────────────────────────────────────────────────────────
-- merge_candidates: account for candidate_documents (CV/CL history) —
-- repoint the duplicate's document rows onto the primary, and drop the
-- now-invalid COALESCE references to cv_storage_key/cv_original_name/
-- cl_storage_key/cl_original_name (dropped from recruitment.candidates in
-- 20260728000005_candidate_documents.sql). Same signature and return
-- columns as before — body-only change.
-- Migration: 20260728000006_merge_candidates_document_history.sql
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ─── Candidate documents (CV/CL history) ──────────────────────────────────
  -- Captured before the repoint so the scalar merge below can still tell
  -- whether primary had a CV of its own prior to merging (used to decide
  -- whose parse output wins). Application code prunes the primary back down
  -- to 3 documents per type afterward — pure SQL can delete excess rows but
  -- can't delete the underlying Storage objects, so that step lives in the
  -- API route that calls this function, not here.
  v_primary_had_cv := EXISTS (
    SELECT 1 FROM recruitment.candidate_documents WHERE candidate_id = p_primary_id AND doc_type = 'cv'
  );
  UPDATE recruitment.candidate_documents SET candidate_id = p_primary_id WHERE candidate_id = p_duplicate_id;
  ...
  SET
    ...
    cv_parse_status  = CASE WHEN NOT v_primary_had_cv THEN v_dup.cv_parse_status ELSE cv_parse_status END,
    cv_parsed_by     = CASE WHEN NOT v_primary_had_cv THEN v_dup.cv_parsed_by    ELSE cv_parsed_by    END,
    cv_parsed_at     = CASE WHEN NOT v_primary_had_cv THEN v_dup.cv_parsed_at    ELSE cv_parsed_at    END,
