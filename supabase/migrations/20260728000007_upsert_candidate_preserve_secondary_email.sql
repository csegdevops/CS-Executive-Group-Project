-- ─────────────────────────────────────────────────────────────────────────────
-- upsert_candidate: preserve the submitted email as secondary_email on
-- collision, instead of discarding it. Previously, when a submission matched
-- an existing candidate via phone (or secondary_email) but used a different
-- primary email, that new email was silently dropped — never written to
-- either email column. Now it fills secondary_email if that slot is free.
-- Migration: 20260728000007_upsert_candidate_preserve_secondary_email.sql
-- Same 15-param signature — body-only change.
-- ─────────────────────────────────────────────────────────────────────────────
