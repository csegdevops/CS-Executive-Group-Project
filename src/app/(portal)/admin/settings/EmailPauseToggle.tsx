...
export function EmailPauseToggle({ initialPaused }: { initialPaused: boolean }) {
  ...
  async function toggle() {
    ...
    try {
      const res = await fetch("/api/admin/system-settings", {
        ...
        body: JSON.stringify({ emails_paused: next }),
      })
      if (!res.ok) {
      ...
      setPaused(next)
      toast.success(next ? "All outbound emails are now paused" : "Emails resumed")
      router.refresh()
    } catch (err) {
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-3 min-w-0">
        <MailWarning className={cn("h-5 w-5 shrink-0", paused ? "text-amber-600" : "text-muted-foreground")} />
        <div className="min-w-0">
          <p className="font-medium text-sm">Pause all emails</p>
          <p className="text-xs text-muted-foreground mt-0.5">
            Stops notification and invite emails (consultations, jobs, timesheets, contractor/supervisor invites, admin password resets) from actually sending — they&apos;re logged, not delivered.
            Does not cover a user&apos;s own &quot;Forgot password&quot; request, which goes straight through Supabase Auth.
          </p>
        </div>
      </div>
      ...
      <button
        onClick={toggle}
        disabled={pending}
        aria-label={paused ? "Resume emails" : "Pause all emails"}
        className={cn(
