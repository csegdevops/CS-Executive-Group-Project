...
// GET /api/recruitment/candidates/[candidateId]/cv?type=cv|cl
export async function GET(req: NextRequest, { params }: { params: Promise<{ candidateId: string }> }) {
  ...
  const { data: candidate } = await (admin.schema("recruitment") as any)
    .from("candidates")
    .select("first_name, last_name, cv_storage_key, cl_storage_key")
    .eq("id", candidateId)
    .single()
  ...
  return buildCvDownloadResponse(storageKey, candidate.first_name, candidate.last_name, docType)
}
